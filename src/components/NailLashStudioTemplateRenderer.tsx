...
const TESTIMONIALS = [
  { name: 'Ananya R.', initials: 'AR', detailKey: 'review1Detail', quoteKey: 'review1Quote' },
  { name: 'Maya K.', initials: 'MK', detailKey: 'review2Detail', quoteKey: 'review2Quote' },
  { name: 'Sana P.', initials: 'SP', detailKey: 'review3Detail', quoteKey: 'review3Quote' },
] as const;
...
function Button({ href, children, primary = false, t }: { href: string; children: ReactNode; primary?: boolean; t: NailLashSurface }) {
  return (
    <a
      href={href}
...
export default function NailLashStudioTemplateRenderer({ data, mode }: Props) {
  ...
  const S = siteText('nail_lash_studio', locale);

  const nameStyle = { ...getSalonNameStyle(data) };
  if (!nameStyle.color) nameStyle.color = '#fffaf7';
  ...
  return (
    <div className={`shadow-2xl border flex flex-col overflow-hidden transition-all duration-500 origin-top mx-auto h-full ${mode === 'desktop' ? 'w-full max-w-[980px] rounded-2xl' : 'w-[375px] max-w-full max-h-[812px] rounded-[2rem] border-[8px]'}`} style={{ borderColor: mode === 'desktop' ? line : ink, backgroundColor: t.page }}>
      {mode === 'desktop' ? (
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar" style={{ backgroundColor: cream, color: ink }}>
        {/* Header and navigation — Phase 10.1 global header (nail & lash design, keeps its pink strip) */}
        <SiteHeader themeId="nail_lash_studio" data={data} mode={mode} />
        ...
        {/* Nail Art Showcase */}
        <section id="section-nail-art" className="px-5 md:px-8 py-12" style={{ backgroundColor: t.artBand }}><div className="flex flex-col md:flex-row md:items-end justify-between gap-5 mb-8"><SectionTitle eyebrow={S.nailArtEyebrow} title={S.nailArtTitle} body={S.nailArtBody} light t={t} /><a href="#section-contact" className="inline-flex items-center gap-2 rounded-full px-4 py-2.5 text-[9px] font-extrabold uppercase tracking-[0.18em] self-start" style={{ backgroundColor: pink, color: '#ffffff' }}>{S.startYourSet} <ArrowRight className="w-3.5 h-3.5" /></a></div><div className={`grid gap-3 ${mode === 'desktop' ? 'grid-cols-3' : 'grid-cols-2'}`}>{NAIL_ART.map((item, index) => <ImageShowcaseCard key={item.name} item={item} index={index} />)}</div></section>

        {/* Gel / Acrylic Showcase */}
        <section id="section-gel-acrylic" className="px-5 md:px-8 py-12" style={{ backgroundColor: sand }}><div className="grid md:grid-cols-[0.82fr_1.18fr] gap-8 items-center"><div><SectionTitle eyebrow={S.gelEyebrow} title={S.gelTitle} body={S.gelBody} t={t} /><div className="flex flex-wrap gap-2 mt-6"><span className="rounded-full px-3 py-2 text-[9px] font-extrabold" style={{ backgroundColor: t.card, color: pinkDeep }}>{S.gelChip1}</span><span className="rounded-full px-3 py-2 text-[9px] font-extrabold" style={{ backgroundColor: t.card, color: pinkDeep }}>{S.gelChip2}</span><span className="rounded-full px-3 py-2 text-[9px] font-extrabold" style={{ backgroundColor: t.card, color: pinkDeep }}>{S.gelChip3}</span></div></div><div className="grid grid-cols-3 gap-2">{GEL_ACRYLIC.map((item, index) => <ImageShowcaseCard key={item.name} item={item} index={index} compact />)}</div></div></section>
        ...
        {/* Lash & Brow */}
        <section id="section-lash-brow" className="px-5 md:px-8 py-12" style={{ backgroundColor: pinkSoft }}><div className="grid md:grid-cols-[1fr_1.2fr] gap-8 items-center"><div><SectionTitle eyebrow={S.lashEyebrow} title={S.lashTitle} body={S.lashBody} t={t} /><div className="mt-6 rounded-2xl p-4 border" style={{ borderColor: line, backgroundColor: t.card }}><div className="flex items-center gap-3"><span className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: t.artBand, color: pink }}><Eye className="w-4 h-4" /></span><div><p className="text-xs font-extrabold" style={{ color: ink }}>{S.lashNoteTitle}</p><p className="text-[9px] mt-1" style={{ color: muted }}>{S.lashNoteBody}</p></div></div></div></div><div className="grid grid-cols-3 gap-2">{LASH_BROW.map((item, index) => <ImageShowcaseCard key={item.name} item={item} index={index} compact />)}</div></div></section>

        {/* Gallery */}
        <section id="section-gallery" className="px-5 md:px-8 py-12" style={{ backgroundColor: white }}><div className="flex items-end justify-between gap-4 mb-7"><SectionTitle eyebrow={S.galleryEyebrow} title={S.galleryTitle} body={S.galleryBody} t={t} /><a href={data.socialProfiles?.instagram || '#section-gallery'} className="hidden md:inline-flex items-center gap-2 text-[9px] font-extrabold uppercase tracking-[0.18em]" style={{ color: pinkDeep }}>{S.followTheEdit} <Instagram className="w-4 h-4" /></a></div><div className={`grid gap-3 ${mode === 'desktop' ? 'grid-cols-5' : 'grid-cols-2'}`}>{GALLERY_IMAGES.map((item, index) => { const label = [S.gallery1, S.gallery2, S.gallery3, S.gallery4, S.gallery5][index] || S.gallery1; return <div key={item.alt} className={`${mode === 'desktop' && index === 0 ? 'md:row-span-2 min-h-[300px]' : 'min-h-[145px]'} relative rounded-[1.25rem] overflow-hidden group`}><img src={item.image} alt={item.alt} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" /><div className="absolute inset-0 bg-gradient-to-t from-[#211b24]/75 to-transparent" /><span className="absolute left-3 bottom-3 text-[9px] font-extrabold uppercase tracking-[0.14em] text-white">{label}</span></div>; })}</div></section>

        {/* About Studio */}
        <section id="section-about" className="px-5 md:px-8 py-12" style={{ backgroundColor: sand }}><div className="grid md:grid-cols-[1fr_1fr] gap-8 items-center"><div className="relative min-h-[275px]"><div className="absolute left-3 right-3 top-3 bottom-[-8px] rounded-[2rem] rotate-3" style={{ backgroundColor: pink }} /><img src={studioImage} alt="Inside the Nail & Lash Studio" className="relative w-full h-[275px] object-cover rounded-[2rem] border-4 border-white shadow-xl -rotate-2" /><div className="absolute left-[-5px] top-10 rounded-2xl px-3 py-2.5 shadow-lg border" style={{ borderColor: line, backgroundColor: t.card }}><Award className="w-4 h-4" style={{ color: pink }} /><p className="text-[9px] font-extrabold mt-1" style={{ color: ink }}>{S.aboutBadge1}<br />{S.aboutBadge2}</p></div></div><div><SectionTitle eyebrow={S.aboutEyebrow} title={S.aboutTitle} body={data.about || S.aboutFallbackBody} t={t} /><div className="grid grid-cols-3 gap-2 mt-7">{[{ value: '01', label: S.aboutStat1Label }, { value: '∞', label: S.aboutStat2Label }, { value: '5★', label: S.aboutStat3Label }].map((stat) => <div key={stat.label} className="rounded-2xl p-3 border" style={{ borderColor: line, backgroundColor: t.card }}><p className="text-xl font-extrabold" style={{ color: pinkDeep }}>{stat.value}</p><p className="text-[8px] uppercase tracking-[0.12em] font-bold mt-1" style={{ color: muted }}>{stat.label}</p></div>)}</div></div></div></section>

        {/* Team */}
        <section id="section-team" className="px-5 md:px-8 py-12" style={{ backgroundColor: cream }}><div className="flex items-end justify-between gap-4 mb-7"><SectionTitle eyebrow={S.teamEyebrow} title={S.teamTitle} body={S.teamSubtitle} t={t} /><BadgeCheck className="hidden md:block w-8 h-8" style={{ color: pink }} /></div>{publicTeam.length > 0 ? <div className={`grid gap-3 ${mode === 'desktop' ? 'grid-cols-3' : 'grid-cols-2'}`}>{publicTeam.slice(0, 6).map((member) => <TeamCard key={member.id} member={member} t={t} />)}</div> : <div className="rounded-2xl border border-dashed p-8 text-center" style={{ borderColor: sandDeep, backgroundColor: sand }}><Users className="w-8 h-8 mx-auto" style={{ color: pinkDeep }} /><p className="text-sm font-extrabold mt-3" style={{ color: ink }}>{S.teamEmptyTitle}</p><p className="text-xs mt-1" style={{ color: muted }}>{S.teamEmptyBody}</p></div>}</section>

        {/* Testimonials */}
        <section id="section-testimonials" className="px-5 md:px-8 py-12" style={{ backgroundColor: t.artBand }}>
          <SectionTitle eyebrow={S.testimonialsEyebrow} title={S.testimonialsTitle} body={S.testimonialsBody} light center t={t} />
          <div className={`grid gap-3 ${mode === 'desktop' ? 'grid-cols-3' : 'grid-cols-1'}`}>
            {TESTIMONIALS.map((item) => (
              <article key={item.name} className="rounded-[1.5rem] border p-5 flex flex-col" style={{ borderColor: 'rgba(255,255,255,0.14)', backgroundColor: 'rgba(255,255,255,0.06)' }}>
                <div className="flex items-center justify-between"><div className="flex gap-0.5">{[0, 1, 2, 3, 4].map((star) => <Star key={star} className="w-3 h-3" style={{ color: pink, fill: pink }} />)}</div><Quote className="w-5 h-5" style={{ color: pinkGlow }} /></div>
                <p className="text-xs leading-relaxed mt-4 flex-1 text-white/80">“{S[item.quoteKey]}”</p>
                <div className="flex items-center gap-2 mt-5 pt-3 border-t" style={{ borderColor: 'rgba(255,255,255,0.14)' }}><span className="w-8 h-8 rounded-full flex items-center justify-center text-[9px] font-extrabold" style={{ backgroundColor: pink, color: '#ffffff' }}>{item.initials}</span><div><p className="text-[10px] font-extrabold text-white">{item.name}</p><p className="text-[9px] mt-0.5 text-white/50">{S[item.detailKey]}</p></div></div>
              </article>
            ))}
          </div>
        </section>
