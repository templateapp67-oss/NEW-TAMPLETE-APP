...
function statusChipStyle(group: TodayStatusGroup, palette: TodayPalette): CSSProperties {
  switch (group) {
    case 'confirmed':
    ...
    <div className="space-y-4" data-testid="today-appointments">
      ...
      <ul className="space-y-3" role="list">
        {appointments.map((row: TodayAppointment) => (
          <li key={row.id}>
            <article
              data-testid={`today-appointment-${row.bookingId}`}
            ...
            >
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span
                  ...
                  data-testid={`today-time-${row.bookingId}`}
                >
                ...
                <span
                  data-testid={`today-status-${row.bookingId}`}
                  className="rounded-lg px-2.5 py-1 text-[10px] font-black uppercase tracking-wider"
                  style={statusChipStyle(row.statusGroup, palette)}
                >
                  {t(`today.status.${row.status}`)}
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <Field
                  palette={palette}
                  testId={`today-customer-${row.bookingId}`}
                  icon={<User className="h-3 w-3" />}
                  label={t('today.field.customer')}
                >
                  {row.customerName || t('today.noCustomerName')}
                </Field>
                <Field
                  palette={palette}
                  testId={`today-services-${row.bookingId}`}
                  icon={<Sparkles className="h-3 w-3" />}
                  label={t('today.field.services')}
                >
                  {row.serviceNames.join(' + ')}
                </Field>
                <Field
                  palette={palette}
                  testId={`today-duration-${row.bookingId}`}
                  icon={<Clock className="h-3 w-3" />}
                  label={t('today.field.duration')}
                >
                  {formatDurationLabel(row.durationMinutes, t('today.unit.hour'), t('today.unit.minute'))}
                </Field>
                <Field
                  palette={palette}
                  testId={`today-payment-${row.bookingId}`}
                  icon={<CreditCard className="h-3 w-3" />}
                  label={t('today.field.paymentStatus')}
                >
                  {t(`today.payment.${row.paymentStatus}`)}
                </Field>
              </div>
              ...
              >
                <span data-testid={`today-total-${row.bookingId}`}>
                  {t('today.field.total')}:{' '}
                  <b style={{ color: palette.text }}>{formatCurrency(row.total)}</b>
                </span>
                {hasAdvancePaid(row) && (
                  <span data-testid={`today-advance-${row.bookingId}`}>
                    {t('today.field.advance')}:{' '}
                    <b style={{ color: palette.text }}>{formatCurrency(row.advancePaid)}</b>
                  </span>
                )}
                {hasRemainingBalance(row) && (
                  <span data-testid={`today-remaining-${row.bookingId}`}>
                    {t('today.field.remaining')}:{' '}
                ...
                {row.staffName && (
                  <span className="flex items-center gap-1.5" data-testid={`today-staff-${row.bookingId}`}>
                    <Users className="h-3 w-3" style={{ color: palette.accent }} />
                    {row.staffName}
                  </span>
                )}
                <span className="ms-auto">
                  {t('today.field.bookingId')}:{' '}
              ...
              {row.cancelled && (
                <p
                  data-testid={`today-cancelled-note-${row.bookingId}`}
                  className="text-[11px] font-bold"
                  style={{ color: palette.muted }}
                >
                  {t('today.cancelledNote')}
                </p>
              )}
            </article>
          </li>
