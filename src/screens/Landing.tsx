        ...
        <div className="flex-grow overflow-y-auto p-4 md:p-8 pb-20">
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'payments' && (
              ...
              >
                <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-2xs">
                  <h3 className="font-bold text-gray-900 text-sm mb-1">Financial Reconciliation</h3>
                  <p className="text-xs text-gray-400 mb-6">Calculate advance payment deposits and pending balances</p>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200 text-center">
                      <p className="text-[10px] font-bold text-gray-400 uppercase">Gross Booking Value</p>
                      <p className="text-2xl font-extrabold text-gray-900 mt-1">₹{totalBookingsValue.toLocaleString()}</p>
                    </div>
                    <div className="bg-emerald-50/50 rounded-2xl p-4 border border-emerald-100 text-center">
                      <p className="text-[10px] font-bold text-emerald-600 uppercase">Advance Collected</p>
                      <p className="text-2xl font-extrabold text-emerald-800 mt-1">₹{totalAdvanceCollected.toLocaleString()}</p>
                    </div>
                    <div className="bg-rose-50/50 rounded-2xl p-4 border border-rose-100 text-center">
                      <p className="text-[10px] font-bold text-rose-600 uppercase">Remaining Balance</p>
                      <p className="text-2xl font-extrabold text-rose-800 mt-1">₹{totalRemainingAtSalon.toLocaleString()}</p>
                    </div>
                  </div>

                  <h4 className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-3">Recent Transactions</h4>
                  <div className="space-y-2">
                    {appointments.filter(a => a.depositPaid > 0).map(appt => (
                      <div key={appt.id} className="flex justify-between items-center p-3 rounded-xl border border-gray-100 bg-white hover:bg-gray-50/50 transition-colors">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-600">
                            <DollarSign className="w-4 h-4" />
                          </div>
                          <div>
                            <div className="text-xs font-bold text-gray-900">Advance from {appt.customerName}</div>
                            <div className="text-[10px] text-gray-400 font-medium">Scheduled for {appt.time}</div>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-xs font-bold text-emerald-600">+₹{appt.depositPaid}</div>
                          <span className="text-[9px] font-bold uppercase bg-emerald-50 text-emerald-700 px-1.5 rounded">Paid</span>
                        </div>
                      </div>
                    ))}
                  </div>
